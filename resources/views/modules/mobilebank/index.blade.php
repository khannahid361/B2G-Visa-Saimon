        ...
        <div class="card card-custom gutter-b">
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar">
                    ...
                    <a href="javascript:void(0);" onclick="showFormModal('Add New Mobile Bank','Save')" class="btn btn-primary btn-sm font-weight-bolder">
...
<script>
    ...
    $(document).ready(function(){
        ...
        table = $('#dataTable').DataTable({
            ...
            "ajax": {
                ...
                "data": function (data) {
                    ...
                    data.showroom_id = $("#form-filter #showroom_id").val();
                    data._token         = _token;
                }
            },
            "columnDefs": [
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if(data.status == 'error'){
                            ...
                            $('#store_or_update_form #showroom_id').val(data.showroom_id);
        ...
        $(document).on('change', '#showroom_id', function () {
            ...
            if (id) {
                $.ajax({
                    url: "{{ route('bank.get.employee') }}",
                    type: "POST",
                    data: { id: id, _token: _token },
                    dataType: "JSON",
                    success: function (data) {
                        if (data.length > 0) {
                            let html = '<option value="">Select Please</option>';
                            $.each(data, function(index, employee) {
                                html += '<option value="' + employee.id + '">' + employee.name + ' - ' + employee.phone + '</option>';
                            });
                            $('#store_or_update_form #employee_id').html(html);
                            $('#store_or_update_form .selectpicker').selectpicker('refresh');
                        }
                    },
                    error: function (xhr, ajaxOption, thrownError) {
                ...
                $('#store_or_update_form #employee_id').html('<option value="">Select Please</option>');
                $('#store_or_update_form .selectpicker').selectpicker('refresh');
            }
        });
    });
