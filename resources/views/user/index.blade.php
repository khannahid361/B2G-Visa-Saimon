...
$(document).ready(function(){
    $(document).on('change','.departmentRole',function(){
        let html;
        let departmentId = $(this).find('option:selected').val();
        let roleId   = $(this).data('role');
        if(departmentId != ''){
            $.ajax({
                url      : "{{url('department-wise-role')}}/" + departmentId,
                type     : 'GET',
                dataType : "JSON",
                success  : function(data){
                    if(data != ''){
                        ...
                        $.each(data, function(key, value) { html += '<option value="'+ value.department_id +'">'+ value.role_name +'</option>'; });
                        $('#'+ roleId +'').empty();
                        $('#'+ roleId +'').append(html);
